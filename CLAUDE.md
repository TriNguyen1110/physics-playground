...
**Palette (locked, replaces earlier per-module colors):** maroon, toned-down/desaturated cyan,
silver, black, white. Every module draws its glow/accent colors from this palette instead of
each module inventing its own hue — this is what makes the three modules and the World Labs
backdrop read as one coherent thing instead of two unrelated pieces bolted together. Maroon and
toned-down cyan are the two accent colors (split across modules/elements however reads best —
e.g. maroon for one module's primary glow, cyan for another's, silver/white for neutral
structure, black for the void), not a rule that every module must use all five.

**The World Labs generated world and the physics interactive scene must be ONE merged scene,
not two separate experiences.** Currently the Marble-generated world only exists as an external
link (marble.worldlabs.ai) separate from the app — that reads as two disconnected things. Import
the actual generated splat (`.spz` file, URL comes from `lib/api/worldlabs.ts`'s
`startWorldGeneration`/`pollWorldGeneration`, see BOARD.tsv facts) into the SAME R3F `<Canvas>`
that renders the physics modules, using Spark (`@sparkjs/*` per sparkjs.dev, built on Three.js —
see the `spark-react-r3f` starter referenced in [[reference_worldlabs_starter_kit]] memory / the
hackathon resources doc) as the room/environment backdrop the physics objects sit inside, replacing
the flat dark-void background. Recolor/relight the splat's rendering (or select/regenerate a
splat whose own palette) toward the locked palette above rather than leaving it in whatever
colors Marble generated by default, so backdrop and foreground physics genuinely match.
